# Experiment Data Model: From Phenotypes to a Reusable Study

## Why this capability exists

The 0.5.0 experiment layer connects study identity, experimental design,
files, observed variables, germplasm, sensors, longitudinal phenotypes,
and provenance without forcing large data into memory.
...
- [`pheno_experiment()`](https://wep69.github.io/OmniPhenoR/reference/pheno_experiment.md)
- [`pheno_as_experiment()`](https://wep69.github.io/OmniPhenoR/reference/pheno_as_experiment.md)
- [`pheno_register()`](https://wep69.github.io/OmniPhenoR/reference/pheno_register.md)
- [`pheno_query()`](https://wep69.github.io/OmniPhenoR/reference/pheno_query.md)
- [`pheno_schema()`](https://wep69.github.io/OmniPhenoR/reference/pheno_schema.md)
- [`pheno_schema_validate()`](https://wep69.github.io/OmniPhenoR/reference/pheno_schema_validate.md)

## Conceptual workflow

![Conceptual 0.5.0
workflow](../inst/extdata/vignette-results/figures/v33-experiment-data-model.png)
...
design <- data.frame(
  plot_id = paste0("P", 1:6),
  treatment = rep(c("Control", "Drought"), each = 3),
  block = rep(1:3, 2)
)

exp <- pheno_experiment(
  study = "soybean_drought_2026",
  title = "RGB phenotyping of soybean under drought",
  root = tempdir(),
  metadata = list(
    description = "Repeated RGB imaging under controlled water deficit",
    year = 2026,
    crop = "soybean"
  ),
  design = design
...
exp <- pheno_register(
  exp, "variable",
  pheno_variable(
    "cc_exg", "green canopy cover",
    "ExG + Otsu segmentation", "proportion", "1"
  )
)
exp <- pheno_register(
  exp, "germplasm",
  pheno_germplasm("G1", "Cultivar A", "Glycine max")
)
exp
#> <pheno_experiment> soybean_drought_2026 
#>   id: exp_c2c42f71dbeb  schema: 0.5 
#>   tables: 0  files: 0 
#>   variables: 1  germplasm: 1
```
...
#> 1 file_63f01a… trai… phen… csv       38 2bb75… NA             NA      2026-08-2…
pheno_verify_manifest(portable)
#> # A tibble: 1 × 6
#>   file_id           path     exists checksum_ok current_sha256 registered_sha256
#>   <chr>             <chr>    <lgl>  <lgl>       <chr>          <chr>            
#> 1 file_63f01ac88544 traits.… TRUE   TRUE        2bb75ff315e6b… 2bb75ff315e6b251…
```
...
verification sources. The relevant sources here include Papoutsoglou et
al. (2020) MIAPPE community (2026).

# Large Phenotype Tables with Parquet, Arrow, and DuckDB

## Why this capability exists

Large object-level and longitudinal phenotype tables should not be
loaded wholesale when a filtered columnar or database query is
sufficient. Arrow and DuckDB are optional backends, while CSV/RDS remain
available for lightweight use.
...
- [`pheno_table_write()`](https://wep69.github.io/OmniPhenoR/reference/pheno_table_write.md)
- [`pheno_table_read()`](https://wep69.github.io/OmniPhenoR/reference/pheno_table_read.md)
- [`pheno_dataset()`](https://wep69.github.io/OmniPhenoR/reference/pheno_dataset.md)
- [`pheno_db()`](https://wep69.github.io/OmniPhenoR/reference/pheno_db.md)
- [`pheno_db_register()`](https://wep69.github.io/OmniPhenoR/reference/pheno_db_register.md)
- [`pheno_db_query()`](https://wep69.github.io/OmniPhenoR/reference/pheno_db_query.md)
- [`pheno_query()`](https://wep69.github.io/OmniPhenoR/reference/pheno_query.md)

## Conceptual workflow

![Conceptual 0.5.0
workflow](../inst/extdata/vignette-results/figures/v35-parquet-arrow-duckdb.png)
...
traits <- data.frame(
  plot_id = rep(paste0("P", 1:4), each = 3),
  day = rep(c(0, 7, 14), 4),
  canopy_cover = seq(.08, .88, length.out = 12)
)

# Core formats require no database engine.
rds_path <- tempfile(fileext = ".rds")
pheno_table_write(traits, rds_path, backend = "rds")
pheno_table_read(rds_path, backend = "rds")
#>    plot_id day canopy_cover
#> 1       P1   0    0.0800000
#> 2       P1   7    0.1527273
#> 3       P1  14    0.2254545
#> 4       P2   0    0.2981818
...
# In-memory query uses the same semantic filter idea.
pheno_query(traits, plot_id = "P2")
#> # A tibble: 3 × 3
#>   plot_id   day canopy_cover
#>   <chr>   <dbl>        <dbl>
#> 1 P2          0        0.298
...
# Optional larger-than-memory workflows:
# parquet_path <- tempfile(fileext = ".parquet")
# pheno_table_write(traits, parquet_path, backend = "parquet")
# ds <- pheno_dataset(parquet_path, backend = "parquet")
# con <- pheno_db(":memory:")
# pheno_db_register(con, "traits", traits)
# pheno_db_query(con, "SELECT plot_id, AVG(canopy_cover) AS mean_cc FROM traits GROUP BY plot_id")
```
...
#> 1 file_9156d0… trai… phen… csv       38 2bb75… NA             NA      2026-08-2…
pheno_verify_manifest(portable)
#> # A tibble: 1 × 6
#>   file_id           path     exists checksum_ok current_sha256 registered_sha256
#>   <chr>             <chr>    <lgl>  <lgl>       <chr>          <chr>            
#> 1 file_9156d0c7c53f traits.… TRUE   TRUE        2bb75ff315e6b… 2bb75ff315e6b251…
```
...
verification sources. The relevant sources here include Apache Arrow R
package contributors (2026) DuckDB R package contributors (2026).
...
Apache Arrow R package contributors. 2026. *Arrow: Integration to Apache
Arrow*. <https://doi.org/10.32614/CRAN.package.arrow>.

DuckDB R package contributors. 2026. *Duckdb: DBI Package for the DuckDB

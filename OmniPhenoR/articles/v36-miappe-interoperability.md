# MIAPPE 1.2-Aligned Metadata and Interoperability

## Why this capability exists

MIAPPE provides a community metadata model for plant phenotyping
experiments. OmniPhenoR uses a focused, versioned mapping to help users
identify missing metadata and exchange core experiment information while
preserving the official standard as the authority.
...
- [`pheno_miappe_mapping()`](https://wep69.github.io/OmniPhenoR/reference/pheno_miappe_mapping.md)
- [`pheno_miappe_validate()`](https://wep69.github.io/OmniPhenoR/reference/pheno_miappe_validate.md)
- [`pheno_metadata_missing()`](https://wep69.github.io/OmniPhenoR/reference/pheno_metadata_missing.md)
- [`pheno_miappe_export()`](https://wep69.github.io/OmniPhenoR/reference/pheno_miappe_export.md)
- [`pheno_miappe_import()`](https://wep69.github.io/OmniPhenoR/reference/pheno_miappe_import.md)

## Conceptual workflow

![Conceptual 0.5.0
workflow](../inst/extdata/vignette-results/figures/v36-miappe-interoperability.png)
...
exp <- pheno_experiment(
  "bean_disease_2026",
  "Bean disease progress imaging",
  metadata = list(
    description = "Field experiment with repeated lesion segmentation",
    start_date = "2026-05-10"
  )
)

exp <- pheno_register(exp, "location",
  pheno_location("L1", "Field A", environment = "field"))
exp <- pheno_register(exp, "germplasm",
  pheno_germplasm("G1", "Line 1", "Phaseolus vulgaris"))
exp <- pheno_register(exp, "variable",
  pheno_variable("sev", "disease severity",
                 "lesion area / leaf area", "proportion", "1"))

check <- pheno_miappe_validate(exp)
check$summary
#> # A tibble: 1 × 5
#>   standard version required_missing recommended_missing complete
#>   <chr>    <chr>              <int>               <int> <lgl>   
#> 1 MIAPPE   1.2                    0                   7 TRUE
pheno_metadata_missing(exp)
#> # A tibble: 7 × 5
#>   section   field              requirement source              status           
#>   <chr>     <chr>              <chr>       <chr>               <chr>            
#> 1 Study     Study end date     recommended metadata.end_date   missing_recommen…
#> 2 Study     Study type         recommended metadata.study_type missing_recommen…
#> 3 Person    Person name        recommended people.name         missing_recommen…
#> 4 Person    Person ORCID       recommended people.orcid        missing_recommen…
#> 5 Location  Location latitude  recommended locations.latitude  missing_recommen…
#> 6 Location  Location longitude recommended locations.longitude missing_recommen…
#> 7 Data file Data file link     recommended files.path          missing_recommen…

miappe_object <- pheno_miappe_export(exp, format = "list")
round_trip <- pheno_miappe_import(miappe_object)
round_trip$study
#> [1] "bean_disease_2026"
```
...
#> 1 file_ba5261… trai… phen… csv       38 2bb75… NA             NA      2026-08-2…
pheno_verify_manifest(portable)
#> # A tibble: 1 × 6
#>   file_id           path     exists checksum_ok current_sha256 registered_sha256
#>   <chr>             <chr>    <lgl>  <lgl>       <chr>          <chr>            
#> 1 file_ba52614654e6 traits.… TRUE   TRUE        2bb75ff315e6b… 2bb75ff315e6b251…
```
...
Papoutsoglou, Evangelia A., Daniel Faria, Daniel Arend, et al. 2020.
“Enabling Reusability of Plant Phenomic Datasets with MIAPPE 1.1.” *New
Phytologist* 227 (1): 260–73. <https://doi.org/10.1111/nph.16544>.
